function solution(users, emoticons) {
    ...
    for (let discountCase = 0; discountCase < totalDiscountCase; discountCase++) {
        const curResult = [0, 0];
        const discountCaseString = discountCase.toString(discountLength).padStart(emoticonsLength, '0');
        ...
        users.forEach(([minDiscountPercent, maxSales]) => {
            const userSales = discountedEmoticons.reduce((acc, [discountPercent, discountedPrice]) => {
                return discountPercent >= minDiscountPercent ? acc + discountedPrice : acc;
            }, 0);
