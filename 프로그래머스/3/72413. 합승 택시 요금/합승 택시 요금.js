function solution(n, s, a, b, fares) {
    ...
    for (let via = 0; via < n; via++) {
        const fare = shortestDistance[s - 1][via] + shortestDistance[via][a - 1] + shortestDistance[via][b - 1];
    ...
    for (let via = 0; via < n; via++) {
        for (let start = 0; start < n; start++) {
            for (let end = 0; end < n; end++) {
                ...
                const curDistance = shortestDistance[start][via] + shortestDistance[via][end];
