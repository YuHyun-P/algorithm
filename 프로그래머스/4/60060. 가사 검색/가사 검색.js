function solution(words, queries) {
    ...
    return queries.map((query) => {
        ...
        const [curWords, curQuery] = query.endsWith(WILDCARD) ? [words, query] : [reversedWords, reverse(query)];
        const start = lowerBound(curQuery, curWords, compare);
        const end = upperBound(curQuery, curWords, compare);
        const result = curWords.slice(start, end).filter((word) => word.length === curQuery.length).length;
