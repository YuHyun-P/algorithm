...
function dfs(root, tree, sales) {
    ...
    const withRoot = [sale, ...subtreeDp.map((subResults) => Math.min(...subResults))].reduce((acc, cur) => acc + cur);
    const withoutRoot = Array(subtreeDp.length).fill(withRoot - sale);
    subtreeDp.forEach(([subWithoutRoot, subWithRoot], index) => {
        const min = Math.min(subWithoutRoot, subWithRoot);
        withoutRoot[index] = withoutRoot[index] - min + subWithRoot;
    });
...
function createTree(links) {
    ...
    links.forEach(([parent, child]) => result[parent - PADDING].push(child - PADDING));
    return result;
}