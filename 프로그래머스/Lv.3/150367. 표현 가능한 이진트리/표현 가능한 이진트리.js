function solution(numbers) {
    return numbers.map(toBinaryString).map(toPerfectBinaryTree).map(isValid).map((valid) => valid ? 1 : 0);
}
...
function isValid(binaryString) {
    ...
    return isValid(binaryString.substring(0, rootIndex)) && isValid(binaryString.substring(rootIndex + 1));
}